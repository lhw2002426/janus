...
main(int argc, char *argv[])
{
 ...
 /* Declare and initialize sparse graph structures */
    SG_DECL(sg1); SG_DECL(sg2);//define graph
        ...
        if (scanf("%d",&n) == 1 && n > 0)
        {
         ...
         /* Now make the second graph */
            ...
            for (i = 0; i < n; ++i)
            {
                ...
                if(i == -1)
                {
                    sg2.e[sg2.v[i]] = 0;      /* Clockwise */
                    sg2.e[sg2.v[i]+1] = 0;  /* Anti-clockwise */
                    sg2.e[sg2.v[i]+2] = 0; 
                    continue;
                }
         ...
         /* Label sg1, result in cg1 and labelling in lab1; similarly sg2.
            ...
            for(int i=0;i<n;i++)
            {
                printf("v: %d neighbor: %d %d %d\n",i,sg1.e[sg1.v[i]],sg1.e[sg1.v[i]+1],sg1.e[sg1.v[i]+2]);
            }
            printf("graph2\n");
            for(int i = 0;i < n;i++)
            {
            ...
            for(int i=0;i<n;i++)
            {
                printf("v: %d neighbor: %d %d %d\n",i,sg2.e[sg2.v[i]],sg2.e[sg2.v[i]+1],sg2.e[sg2.v[i]+2]);
            }
