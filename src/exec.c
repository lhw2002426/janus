...
if(np->net->network_type == NET_JUPITER)
    {
      maxmin(&np->dp);
      violations = dataplane_count_violations(&np->dp, builder->expr->promised_throughput);
    }
    else{
      violations = ecmp(np->net,&(builder->expr->mrc_traffic));
    }
...
risk_cost_t exec_plan_cost(
  ...
  for (uint32_t i = 0; i < nmops; ++i) {
    cost_pre = mops[i]->pre(mops[i], net);
    bw_t subplan_cost = 0;
    ...
    for (uint32_t step = 0; step < expr->mop_duration; ++step) {
      ...
      if(net->network_type == NET_JUPITER)
      {
        ...
        violations = ecmp(net,0);
        if(violations == -1)
          cost = -1;
        subplan_cost = cost_pre;
      }
      iter->next(iter);
...
for (uint32_t i = running_time; i < expr->criteria_time->steps && net->network_type == NET_JUPITER; ++i) {
